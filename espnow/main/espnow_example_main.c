...
static void espnow_send_cb(const uint8_t *mac_addr,	esp_now_send_status_t status) {
	...
	if(debug) ESP_LOGI(TAG4, "ENVIO ESPNOW status (ID): %d", status);
	...
	if(status == 0){
		if(debug) ESP_LOGI(TAG4," >> Éxito de entrega");
...
//							}
							...
							if(debug) ESP_LOGI(TAG2,"Instant time = %lu", previous_conv_time);
							if(debug) ESP_LOGI(TAG2, "Unit: %d,_Channel: %d, Filtered_value: %lu", 1, 0, my_reads->adc_read[0].adc_filtered);
						}
